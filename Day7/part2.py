...
def find_subdirectories(node, space_required):
    dir_sizes = []
    for child in node.children:
        if child.is_directory:
            ...
            if size >= space_required:
                ...
                sub_directories = find_subdirectories(child, space_required)
                dir_sizes.extend(sub_directories)
    return dir_sizes
...
def no_space_left_on_device(file_name):
    ...
    at_least_size = 30000000
    current_empty_space = total_disk_available - get_size(tree._root)
    space_required = at_least_size - current_empty_space
    possible_dirs = find_subdirectories(tree._root, space_required)
    print(possible_dirs)
